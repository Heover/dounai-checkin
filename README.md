# 豆奶自动签到

每日自动登录 [dounai.win](https://dounai.win) 并签到，获取免费流量，通过 Server酱3 推送结果到微信。

## 工作流

```
识别验证码（4 位字符/加减乘题）→ 登录 → 访问面板建立会话 → 签到 → Server酱3 推送
                                      ↓ 验证码无效
                          刷新验证码并重试（最多 8 次）
```

## 定时

每天北京时间 **00:07** 自动执行（GitHub Actions cron）。

## 当前网站限制

2026-09-06 已修复算术验证码识别并验证登录成功，但签到接口返回
`is_blocked: true`，提示“检测到签到脚本，今日签到机会已锁定”。
该响应会作为失败结束，不重试签到；登录修复不代表网站允许自动签到，
账户的签到限制需通过网站官方途径确认。

2026-09-07 日志再次确认登录成功后首次签到即返回锁定。脚本现在优先处理
`is_blocked: true` 或锁定文案，即使消息同时包含“验证码错误”也立即终止，
并单独推送“签到已锁定”。此处理不能解除服务端锁定，也不能证明下一日会解锁；
当前每日定时仍启用，建议通过 GitHub Actions 暂停工作流，待网站确认允许的签到方式后再恢复。

## 环境变量

| 变量 | 说明 |
|------|------|
| `DOUNAI_EMAIL` | 豆奶账号邮箱 |
| `DOUNAI_PASSWD` | 豆奶账号密码 |
| `SERVER_UID` | Server酱3 UID（组织级共享） |
| `SERVER_KEY` | Server酱3 SendKey（组织级共享） |

## 本地调试

```bash
# 安装依赖（tesseract.js 用于识别登录验证码）
npm install

# 创建 .env 文件
echo "DOUNAI_EMAIL=your@email.com" > .env
echo "DOUNAI_PASSWD=your_password" >> .env
echo "SERVER_UID=your_uid" >> .env
echo "SERVER_KEY=your_key" >> .env

# 运行
npm run checkin
```

> 首次运行 tesseract.js 会从 CDN 下载 OCR 模型（约 15MB），之后自动缓存。

## 技术栈

- Node.js 20+（tesseract.js 验证码识别，无需 Python 环境）
- GitHub Actions 定时调度
- Server酱3 消息推送
